...
以下のストーリー章（${chapterTitle}）を読んで、社員の理解と自律を深める「深い問い」を1問だけ設計してください。

${extraPrompt}

${
  stepNumber > 1 && previousAnswer
    ? `【前の回答】\n${previousAnswer}\nこの前の回答を踏まえて、さらに深い問いを設計してください。`
    : ''
}

各問いには「なぜこの問いが重要か」という理由も添えてください。
戦略・組織・価値創造・人間理解などに関わる本質的な問いにしてください。
      ...
      messages: [
        {
          role: 'system',
          content:
            'あなたは社員の問いを設計するプロフェッショナルです。出力は必ず厳密なJSON形式（{ question, reason }）で返してください。補足や説明は禁止です。',
        },
        {
          role: 'user',
          content: prompt,
