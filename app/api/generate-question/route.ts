...
/* ========== 章ごとの最大ステップ（Ver4） ========== */
// 0:現状=2問, 1:戦略=6問, 2:像=2問, 3:行動=2問（※構造は維持）
...
/* ========== ガイド（新4章：standard / expert） ========== */
/**
 * 「未来」という語の多用は避け、「これから/次の時代/新しい常識」として表現。
 * 第4章は“内部変革＝経営者から社員への行動メッセージ”を意識した問いに刷新。
 */
type ChapterGuide = { label: string; goal: string; probes: string[] };

const CHAPTER_GUIDE_V4_STANDARD: ChapterGuide[] = [
  // 第1章：なぜ今（変わる必然）
  { label: 'なぜ今（変わる必然）', goal: '「これからの常識」から見た今のズレを特定し、動く必然を作る。', probes: [
    '「これからの常識」から見て、当社のやり方で時代遅れに映る点を1つだけ具体例で挙げてください。',
    'このまま続けた場合に最もまずいインパクトは何ですか？影響するKPI/場面を1つと臨界ラインを示してください。'
  ]},
  // 第2章：どう戦う（次の時代の競争軸）
  { label: 'どう戦う（次の時代の競争軸）', goal: 'これからの顧客に選ばれ続ける戦い方を、強み再定義とトレードオフで描く。', probes: [
    'これからの顧客が抱える新しい面倒/ロスを1つ。対象と単位（時間/金額/手間）で具体化してください。',
    'その課題に最短で効く当社の資産/能力/関係は何ですか？1つに絞って再定義してください。',
    'その価値で顧客の体験はどう変わりますか？Before→Afterを1行で示してください。',
    '非業界プレイヤーを含む対抗シナリオを1つ想定し、価値が壊れる条件と設計変更を1点だけ示してください。',
    '実装制約（人/資本/プロセス）を踏まえ、最小GTMウェッジ（誰に/何を/いくら/導入LT）を定義してください。',
    '検証用KRを1〜2個（数値・期限）と、撤退KILLを1つ（条件・期限）決めてください。'
  ]},
  // 第3章：どんな姿を目指すか（これからのあり方）
  { label: 'どんな姿を目指すか（これからのあり方）', goal: '会社・顧客・社会の“これからの姿”を体験で描き、希望を共有する。', probes: [
    '「これからの当たり前」の中で、顧客が当社に「ありがとう」と言う1シーンを描写してください（人物/場所/価値の見え方）。',
    'その姿に近づいているサインを1つ定義し、観測方法/頻度/閾値を記してください。'
  ]},
  // 第4章：どう行動する（変化を起こす文化）
  { label: 'どう行動する（変化を起こす文化）', goal: '越境・挑戦・共創が当たり前になる行動文化を、経営の言葉で示す。', probes: [
    '社員が自部門に閉じず越境して動けるよう、経営としてどんな支援やメッセージを出しますか？具体施策を1つ。',
    '挑戦と共創を日常化するために、いま社内で「やめる慣習」を1つと「始める仕組み」を1つ（頻度/対象）挙げてください。'
  ]},
];

const CHAPTER_GUIDE_V4_EXPERT: ChapterGuide[] = [
  // 第1章
  { label: 'なぜ今（変わる必然）', goal: '外部変化→KPIインパクト→臨界域を明確にする。', probes: [
    '過去4〜8Qで構造変化が顕著な指標を1つ（例：解約率/粗利/在庫回転）。推移と要因を簡潔に説明してください。',
    '変化が継続した場合の影響を1つのKPIで定量化（△◯%/◯件、期限◯年◯Q）。臨界域を定義してください。'
  ]},
  // 第2章
  { label: 'どう戦う（次の時代の競争軸）', goal: '顧客Job×自社Moat×制約×DisruptionでウェッジとKR/KILLを定義。', probes: [
    '優先セグメントと最大のPain/Jobを1つ。経済負担（時間/金額）で具体化してください。',
    '最短で効く自社資産/能力/関係を特定し、価値式（成果＝現状−痛み＋強み）を1行で示してください。',
    '代替/新技術で価値式が壊れる条件を1つ。回避の設計変更を1点挙げてください。',
    '実装制約を踏まえた最小GTMウェッジ（誰に/何を/いくら/導入LT）を定義してください。',
    '検証用KRを2個（数値・期限）と撤退KILL1個（条件・期限）を決めてください。',
    '優先しない領域/施策を1つ（トレードオフ）。見直し期日も明記してください。'
  ]},
  // 第3章
  { label: 'どんな姿を目指すか（これからのあり方）', goal: '顧客現場1シーン＋外部Leadingで検証可能にする。', probes: [
    '3年後の顧客現場を1シーンで描写（人物/場所/当社の価値の見え方）。',
    '外部Leading指標を1つ（例：標準採用の打診）。観測方法/周期/閾値を定義してください。'
  ]},
  // 第4章（内部変革を経営の言葉で）
  { label: 'どう行動する（変化を起こす文化）', goal: '自律分散と越境を支える制度設計を明確化。', probes: [
    '承認を待たずに動ける自律チームの設計（権限/予算/期間）を1つ提案してください。',
    '挑戦・越境・共創を評価/報酬に反映する基準を1つ具体化し、レビュー頻度も示してください。'
  ]},
];
...
const CONTRADICT_SEEDS = {
  ...
  2: [
    ...
    '像の検証として、現場で観測できる小さな兆し（例：標準採用の打診、NPSコメントの変化）を1つ定義してください。',
  ],
  3: [
...
/* ========== SYSTEM_PROMPT（新4章仕様） ========== */
const SYSTEM_PROMPT = `
あなたは経営者に伴走する「次の時代をつくる」戦略ファシリテーターです。
与えられる情報（SEEDテーマ / 直前回答 / Q&A履歴 / 財務サマリ / LENSヒント）を使い、
重複させず、直前回答の具体語を必ず含める「次の1問だけ」を提示してください。

【章別ステップ上限】
- なぜ今(0): 2問（「これからの常識」から見たズレ / 継続時のKPIリスク）
- どう戦う(1): 6問（顧客の新しい面倒 / 強み再定義 / 体験Before→After / 対抗シナリオ / 最小GTM / KR&KILL）
- どんな姿を目指すか(2): 2問（顧客の1シーン / 外部シグナル）
- どう行動する(3): 2問（越境を促す経営メッセージ / やめる慣習＆始める仕組み）
...
- JSONのみ {"question":"...","reason":"..."}
- question 50〜120字 / reason 40〜100字
- 観点をずらす（第2章なら：対抗シナリオ/制約/体験Before→After/KR・KILL/顧客像/GTM等)`.trim();
...
必ず {"question":"...","reason":"..."} のJSONのみで返してください。`
    ...
    return new NextResponse(JSON.stringify({ step, meta: {
      ...
      maxSteps: maxStepsForChapter(norm.chapterIndex), // ※ 構造は維持（第2章は常に6）
