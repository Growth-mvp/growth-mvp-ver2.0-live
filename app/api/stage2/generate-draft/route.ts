...
function computeCoverageIssues(
  ...
  if (!/事業ポートフォリオ判断\s*[:：]/.test(ch2)) {
    missing.push(
      `第2章に「事業ポートフォリオ判断：成長牽引軸：...｜収益改善軸：...｜再構築軸：...」を追加する`
    );
  }
...
- 戦略結論では、事業を「成長牽引軸」「収益改善軸」「再構築軸」の3役割に分け、何を変えるかを断定する。
- 事業セグメントがある場合、必ず「成長牽引軸」「収益改善軸」「再構築軸」に分類する。分類理由は財務指標・市場機会・SWOTのいずれかで説明する。
...
- 第2章の冒頭1行目は必ず「戦略結論：」にする。ここに「成長牽引軸」「収益改善軸」「再構築軸」と、それぞれの経営判断を明確に書く。
...
第2章：どう戦う（戦略）
戦略結論：...を成長牽引軸、...を収益改善軸、...を再構築軸と位置づけ、投資・撤退・KPIの判断基準を切り替える。
事業ポートフォリオ判断：成長牽引軸：...｜収益改善軸：...｜再構築軸：...
1) 勝ち筋：...
   狙う価値ドライバー：...
...
- 「事業ポートフォリオ判断」は、入力の事業セグメントがある場合は各セグメント名を使い、成長牽引軸・収益改善軸・再構築軸に分類する
...
【戦略判断MUST】
- 第2章の1行目は必ず「戦略結論：」で始め、成長牽引軸・収益改善軸・再構築軸を明示する
- 「AからBへシフト」のような単純な移行表現は禁止。事業ごとの役割分担として書く
- 第2章の2行目は必ず「事業ポートフォリオ判断：成長牽引軸：...｜収益改善軸：...｜再構築軸：...」にする
...
最重要：第2章の先頭は「戦略結論：」で始め、成長牽引軸・収益改善軸・再構築軸を使って会社固有の資源配分・見直し対象を断定してください。
