...
import { supabase, getCompanyIdFromCookie, setCompanyIdCookie, isValidUUID } from './client';
...
async function resolveCompanyId(userId: string): Promise<string | null> {
  ...
  try {
    ...
    if (m.companyId) {
      try { setCompanyIdCookie(m.companyId); } catch {}
      return m.companyId;
    }
  } catch {}
  return null;
...
export async function saveStoryAnswers2(userId: string, answers2: ChapterAnswers[]) {
  try {
    ...
    if (companyId) {
      // company_id を必ず添えて upsert（同一 user_id で上書き）
      let ins = await supabase
        .from(T_ANSWERS2)
        .upsert([{ ...payload, company_id: companyId }], { onConflict: 'company_id,user_id' });
    ...
    // 会社未所属：user基準で保存（互換）。※ UNIQUE(user_id) 前提で onConflict:user_id
    let ins2 = await supabase.from(T_ANSWERS2).upsert([payload], { onConflict: 'user_id' });
...
export async function loadStoryAnswers2(userId: string): Promise<ChapterAnswers[] | null> {
  try {
    ...
    if (companyId) {
      ...
      if (!r1.error && r1.data) {
        let result: any = (r1.data as any).answers2;
    ...
    if (!r2.error && r2.data) {
      let result: any = (r2.data as any).answers2;
      if (typeof result === 'string') { try { result = JSON.parse(result); } catch { result = []; } }
      if (!Array.isArray(result)) result = [];
      return result;
    }
...
export async function saveProgressLog(
  ...
  try {
    ...
    // RLS 強制下では company_id なしだと弾かれるため、必ず付与
    const rows = [{
      user_id: userId,
      okr_id: okrId,
      company_id: companyId, // ← 重要（NULLでもスキーマが許すなら可。RLS設計に依存）
    ...
    const ins = await supabase.from(T_LOGS).insert(rows);
    if (ins.error) {
      // eslint-disable-next-line no-console
