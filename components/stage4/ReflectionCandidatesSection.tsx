...
export function ReflectionCandidatesSection({
  ...
  const [candidates, setCandidates] = useState<OKRCandidate[]>([]);
  ...
  const fetchCandidates = async () => {
    ...
    try {
      ...
      if (!ok || !sessionData?.session?.access_token) {
        console.warn('[STAGE4] Not authenticated');
        setCandidates([]);
        setLoading(false);
        return;
      }
      ...
      if (!res.ok) {
        console.warn('[STAGE4] Failed to fetch reflection candidates:', res.status);
        setCandidates([]);
        return;
      }
      const data = await res.json();
      console.log('[STAGE4] API response:', data);
      const filtered = (data.candidates || []).filter((c: OKRCandidate) => c.status === 'pending');
      console.log('[STAGE4] Filtered pending candidates:', filtered.length);
      setCandidates(filtered);
    } catch (err) {
      console.error('[STAGE4] fetchCandidates error:', err);
                ...
                <div className="flex items-start justify-between gap-4">
                  <div className="flex-1">
                    ...
                    <p className="mt-2 font-semibold text-slate-950">
                      {candidate.objective || candidate.title}
                    </p>
                    {candidate.key_results && candidate.key_results.length > 0 && (
                      <div className="mt-2">
                        <p className="text-xs font-semibold text-gray-700">
                          Key Results
                        </p>
                        <ul className="mt-1 list-disc space-y-1 pl-5 text-xs text-gray-700">
                          {candidate.key_results.map((kr, idx) => (
                            <li key={idx}>{kr}</li>
                          ))}
                        </ul>
                      </div>
                    )}
                    {(candidate.owner || candidate.due_date) && (
                      <p className="mt-2 text-xs text-gray-600">
                        Owner: {candidate.owner || '-'} / Due: {candidate.due_date || '-'}
                      </p>
                    )}
                  </div>
                </div>
